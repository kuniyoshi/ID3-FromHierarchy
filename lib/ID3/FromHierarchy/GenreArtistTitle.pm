package ID3::FromHierarchy::GenreArtistTitle;
...
Readonly my @SUFFIXES => qw( .mp3 .MP3 );
...
sub parse {
    ...
    my %tag = (
        title   => basename( $pathes[2], @SUFFIXES ),
        track   => undef,
        artist  => $pathes[1],
        album   => undef,
        genre   => $pathes[0],
...
ID3::FromHierarchy::GenreArtistTitle - Perl extension for ID3::FromHierarchy
...
=head1 SYNOPSIS
  ...
  my %tag  = eval {
      ID3::FromHierarchy::GenreArtistTitle->new->parse( $file )
  };
  die $@
      if ( $@ );
  print map { "$_: $tag{$_}\n" } keys %tag;
...
In this module expects the file system has hierarchy like genre - artist - music file.
